...
export class FrameBuffer {
  ...
  private colorInternalFormat: number;
  ...
  constructor(gl: GL, width: number, height: number) {
    ...
    this.width = width;
    this.height = height;
    // 初始默认值，在 createFramebuffer 中会根据支持情况更新
    this.colorInternalFormat = gl.RGBA16F as number;
    this.colorType = ((gl as any).HALF_FLOAT || (gl as any).HALF_FLOAT_OES || 0x140B) as number;
  ...
  private createFramebuffer() {
    ...
    // 优先尝试半浮点纹理（更接近网页渲染效果）
    let internalFormat: number = gl.RGBA16F as number;
    let type: number = ((gl as any).HALF_FLOAT || (gl as any).HALF_FLOAT_OES || 0x140B) as number; // 0x140B == HALF_FLOAT
    gl.texImage2D(
      gl.TEXTURE_2D,
      0,
      internalFormat,
    ...
    // 检查FBO状态，必要时降级为 8bit 纹理，提升兼容性（例如 Remotion 的软件渲染器）
    let status = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
      // 回退到 RGBA8
      internalFormat = gl.RGBA8 as number;
      type = gl.UNSIGNED_BYTE as number;
      gl.bindTexture(gl.TEXTURE_2D, texture);
      gl.texImage2D(
        gl.TEXTURE_2D,
        0,
      ...
      if (status !== gl.FRAMEBUFFER_COMPLETE) {
        // 某些后端对深度纹理不友好：尝试移除深度附件
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, null, 0);
        status = gl.checkFramebufferStatus(gl.FRAMEBUFFER);
        if (status !== gl.FRAMEBUFFER_COMPLETE) {
          throw new Error(`Framebuffer is incomplete after fallback: ${status}`);
        }
      }
    }

    // 记录实际使用的颜色附件格式，供 resize 使用
    this.colorInternalFormat = internalFormat;
    this.colorType = type;
...
export class MultiPassRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, configs: RenderPassConfig[], contextOptions?: WebGLContextAttributes) {
    const gl = canvas.getContext("webgl2", contextOptions ?? {}) as GL | null;
    if (!gl) throw new Error("WebGL 2 not supported");

    // 检查浮点纹理扩展（可选）：没有也能工作，帧缓冲会自动回退为 RGBA8
